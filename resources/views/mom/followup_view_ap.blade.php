				...
				<div id="landingTableDiv" style="margin-top:15%;margin-left:3%;">
					...
					<div class="col-md-8" >	
						<table class="landingTable" style="box-shadow: 10px 10px 5px #888888;">
							<tr class="trHeader">
								<td>Action Point Title</td><td>Status </td>
								<td>Completion Status</td><td>Comment</td><td>Meeting ID</td><td>EDIT</td>
							</tr>	
							@foreach($mom_lists as $mom_list)
								<tr>
									<td>{{$mom_list->mom_title}}</td>
									<td>{{$mom_list->mom_status}}</td>
									<td>{{$mom_list->mom_completion_status}}</td>
									<td>{{$mom_list->comment}}</td>
									<td>{{$mom_list->meeting_id}}</td>
									<td>
										{!! Form::open(array('url' => 'editView', 'method' => 'GET')) !!}
											{!! Form::hidden('meeting_id', $mom_list->meeting_id) !!}
