...
class MomController extends Controller {
	...
	public function change_dept(){
		$sessionDept = Request::get('sessionDept');
		$_SESSION['dept'] =$sessionDept;
	...
	public function is_autenticated(){
		...
		if(isset($_SESSION['dashboard_user_id'])){
			...
			if($adminLists != null){
				...
				if($adminLists[0]->isMultiple == 'true'){
					$_SESSION['ADMIN_DEPT_LISTS'] = $adminLists[0]->depts;
				}
				else{
					$_SESSION['ADMIN_DEPT_LISTS'] = 'false';
				}
				return 'admin';
			}
			else{
	...
	public function landing_view()
	{
		...
		$mom_name_list = array();
		$red_meeting_lists_responsible = array();
			...
			foreach($mom_lists as $mom_list)
			{
				...
				if($min_date==null)
				{
					...
					if (preg_match("/$preg_id/", $mom_list->responsible)){
						...
						if($mom_list->mom_status == 'active'){
							array_push($red_meeting_lists_responsible, $meeting_id_put[$j]);
						}
					}
				}
				else
		...
		return view('mom.momLanding',compact('meeting_lists','current_date','mom_furthest_endtime_list','yellow_date1','yellow_date2','mom_name_list','val','meeting_lists_with_responsible','red_meeting_lists_responsible'));
	}
...
//search and return a combine table data to the views
	public function search_post()
	{
		...
		foreach ($combine_obj_lists as $row) 
		{
			...
			foreach($mom_lists as $mom_list){
				...
				if (preg_match('/'.$_SESSION['email'].'/',$resoponsible_persons)){
					$is_related = true;
				}
			}
			...
			foreach($meeting_lists as $meeting_list){
				...
				if (preg_match('/'.$_SESSION['email'].'/',$attendee_persons)){
					$is_related = true;
				}
				$recipient_persons = $meeting_list->mail_recipient;
				if (preg_match('/'.$_SESSION['email'].'/',$recipient_persons)){
					$is_related = true;
				}
			}
			if($val =='superAdmin' ){
		...
		$searchForExport 		= Request::get('searchForExport');
		$paginateNumber = '10';
		if($searchForExport =='yes'){
			$paginateNumber = '1000';
		}
		if($meeting_id_str != ''){
			$meeting_id_str = trim($meeting_id_str,',');
	...
	public function search_post_all()
	{
		...
		$search_meeting_title 		= '';
		$search_meeting_datetime1 	= '';
		$search_meeting_datetime2 	= '';
		$search_meeting_attendee 	= '';
		$search_mail_recipient 	= '';
		$search_meeting_type 		= '';
		$search_meeting_status 	= '';
		$search_mom_title 			= '';
		$search_responsible 		= '';
		$search_mom_start_time1 	= '';
		$search_mom_start_time2 	= '';
		$search_mom_end_time1 		= '';
		$search_mom_end_time2		= '';
		$search_mom_status 		= '';
	...
	public function checker($meeting_id){
		...
		foreach($meeting_table_lists as $meeting_table_list){
			if(preg_match('/'.$_SESSION['email'].'/',$meeting_table_list->mail_recipient)){
				$is_ok = true;
				return $is_ok;
			}
			if(preg_match('/'.$_SESSION['email'].'/',$meeting_table_list->meeting_attendee)){
				$is_ok = true;
				return $is_ok;
			}
			if($val == 'admin'){
		...
		foreach($mom_table_lists as $mom_table_list){
			if(preg_match('/'.$_SESSION['email'].'/',$mom_table_list->responsible)){
				$is_ok = true;
				return $is_ok;
			}
			//$is_ok = true;
	...
	public function mom_view()
	{
		...
		for($j=0;$j<count($meeting_attendee_arr_temps);$j++){
			...
			foreach($designation_lists as $designation_list){
				if($designation_list->designation != ''){
		...
		for($j=0;$j<count($mom_responsible_arr_temps);$j++){
			...
			$designation_lists = \DB::select(\DB::raw($getDesignationQuery));
			foreach($designation_lists as $designation_list){
				if($mom_responsible_arr_temps[$j] !=''){
					$designation  = $designation_list->designation;
					//echo $designation.'  '. $designation_list->user_name.'<br>';
	    ...
	    for($i=0;$i<count($to_arr);$i++)
	    {
	     ...
	     //    $phpMailer->add_to($to_arr[$i]);
			...
			if($toAddressArr[1] == 'summitcommunications.net' || $toAddressArr[1] == 'summit-centre.com'){
				$messages .= "<p><a href='http://103.15.245.78:8005/mom/public/MailView?meeting_id=".$meeting_id."&login_id=".$toAddressArr[0]."'>Click here</a></p>";
			}
			else{
				$messages .= "<p><a href='http://103.15.245.78:8005/mom/public/guestMOM?meeting_id=".$meeting_id."&login_id=".$toAddressArr[0]."'>Click here</a></p>";
			}
			$messages .= '<br>Thanks';
			$messages .= "</body></html>";
  ...
  //   }
    public function createMail($counTime,$val){
		...
		try{
			...
			$toAddressArr = explode('@', $toAddress);
			if($toAddressArr[1] == 'summitcommunications.net' || $toAddressArr[1] == 'summit-centre.com'){
				$messages .= '<h3>A New MoM created by '.$strArr[3].' please use the below link to see</h3>';
				$url = "http://103.15.245.78:8005/mom/public/MailView?meeting_id=".$strArr[2]."&login_id=".$toAddressArr[0];
				$messages .= $url;
				...
				$url = 'http://172.20.17.50:1347/sendMailMOM';
				$myvars = 'toAddress=' . $data['email'] . '&msg=' . $data['msg'] .'&sub='.$data['sub'];
  ...
  //   	echo '<input type="button" value="Close this window" onclick="windowClose();">';
     ...
     public function mailAfter(){
		...
		$current_date_tomorrow_obj->modify('+2 day');
		$current_date_tomorrow = $current_date_tomorrow_obj->format('Y-m-d H:i:s');
  		$mom_table_t4_lists = \DB::select(\DB::raw("SELECT * FROM mom_db.mom_table WHERE mom_status='active' and end_time > '".$current_date."'"));
  		...
  		foreach($mom_table_t4_lists as $mom_table_t4_list)
  		{
  			...
  			$temp_responsible_arr = explode(',', $mom_table_t4_list->responsible);
	    		...
	    		// 	$phpMailer->phpMailer();
					...
					if(count($to_arr_temp)>1){
  					...
  					$user_list_names = \DB::select(\DB::raw("SELECT email,user_name,designation FROM login_plugin_db.login_table  WHERE email ='".$toArr[$i]."'; "));
	  		...
	  		// 		{
	  					...
	  					$messages = '';
							if(array_key_exists($toArr[$i],$daily_arr)){
								$messages .= '<tr>';
								$messages .= "<td><b>(<a href='http://103.15.245.78:8005/mom/public/view?meeting_id=$mom_table_t4_list->meeting_id' >$mom_table_t4_list->meeting_id</a>)</b> $meeting_title</td>	
					...
					// return '';
			  		...
			  		foreach($daily_arr as $daily){
						...
						$from_super = array_search($daily, $daily_arr);
			  			$msg = 'Dear '.$from_super.',<br> Please be informed that below action points are very close to their deadlines.<br><br>'.$messages_header.$daily.$messages_footer.'Thanks & Best Regards<br> SCL MoM Tool<br><br><img src="http://103.15.245.78:8005/mom/public/images/mom_logo.jpg"><br><br> <i>This is an auto generated email. Please don’t reply. For any query please email to oss@summitcommunications.net.</i><br><br><br>'; 

			  			$sub = 'MOM Action Points very close to their dead line';
			  			
			  			// if($from_super =='md.shariful@summitcommunications.net')
			  			// {
   ...
   public function t4Mail(){
					...
					// return '';
			  		...
			  		foreach($daily_arr as $daily){
			  			$messages_header  = '<style>
							...
							$messages_footer = "</table></body></html><br>";
						$from_super = array_search($daily, $daily_arr);
			  			$msg = 'Dear '.$from_super.',<br> Please be informed that below action points have failed to meet their deadlines.<br><br>'.$messages_header.$daily.$messages_footer.'Thanks & Best Regards<br> SCL MoM Tool<br><br><img src="http://103.15.245.78:8005/mom/public/images/mom_logo.jpg"><br><br> <i>This is an auto generated email. Please don’t reply. For any query please email to oss@summitcommunications.net.</i><br><br><br>'; 

			  			$sub = 'MOM Action Point Failed to meet their dead line';
  				...
  				{	
  					$supervisor_id = $user_table_list->supervisor_email;
						...
						if(array_key_exists($supervisor_id,$daily_arr)){
							$messages .= '<tr>';
							$messages .= "<td><b>(<a href='http://103.15.245.78:8005/mom/public/view?meeting_id=$mom_table_t4_list->meeting_id'>$mom_table_t4_list->meeting_id</a>)</b></td><td> $meeting_title</td>	
											<td>$meeting_datetime</td>
											<td><b>($mom_table_t4_list->mom_id)</b> $mom_table_t4_list->mom_title</td>
											<td>$mom_table_t4_list->start_time</td>
											<td>$mom_table_t4_list->end_time</td>
		...
		//print_r($keys);
  		...
  		foreach($daily_arr as $daily){
  			$messages_header  = "<style>
				...
				$messages_footer = "</table></body></html><br>";
			$from_super = array_search($daily, $daily_arr); 
  			$msg = 'Dear Sir,<br>Please be informed that below action points of your team members (Total:'.substr_count($daily, '<tr>').') have failed to meet their deadlines. <br><br>'.$messages_header.$daily.$messages_footer.'Thanks & Best Regards<br> SCL MoM Tool<br><br><img src="http://103.15.245.78:8005/mom/public/images/mom_logo.jpg"><br><br> <i>This is an auto generated email. Please don’t reply. For any query please email to oss@summitcommunications.net.</i>'; 
  	...
  	public function afterEndTimeMail1()
  	{
  		//return 'adf';
		...
		$current_date_previous = $current_date_previous_obj->format('Y-m-d H:i:s');
  		...
  		foreach($mom_table_t4_lists as $mom_table_t4_list)
  		{
	    	...
	    	// 	$phpMailer->phpMailer();
				...
				// $file = fopen('../TempFiles/test.txt','a');

				// $log_text = "\r\n\r\n".'meeting_id = '.$mom_table_t4_list->meeting_id.' , meeting_title = '.$meeting_title.' , meeting_datetime = '.$meeting_datetime.' , mom_title = '.$mom_table_t4_list->mom_title.' , mom_start_time = '.$mom_table_t4_list->start_time.' , mom_end_time = '.$mom_table_t4_list->end_time.' , mom_comment = '.$mom_table_t4_list->comment.' , person_failed_mom = '.$toArr[$i].' , person_mailing_to ='.$toArr[$i];
	    	...
	    	// 	$phpMailer->phpMailer();
				...
				// $log_text = "\r\n\r\n".'meeting_id = '.$mom_table_t4_list->meeting_id.' , meeting_title = '.$meeting_title.' , meeting_datetime = '.$meeting_datetime.' , mom_title = '.$mom_table_t4_list->mom_title.' , mom_start_time = '.$mom_table_t4_list->start_time.' , mom_end_time = '.$mom_table_t4_list->end_time.' , mom_comment = '.$mom_table_t4_list->comment.' , person_failed_mom = '.$toArr[$i].' , supervisor_person_mailing_to = '.$supervisor_id;
  		...
  		// for($i=0;$i<count($daily_arr);$i++){
  		foreach($daily_arr as $daily){
  			//print_r( array_keys($daily_arr, $daily));
  			$key = array_search($daily, $daily_arr);
  			//echo $key;
  			fputs($file,$key);
  			fputs($file,$daily);
  		}
  ...
  //   	echo '<input type="button" value="Close this window" onclick="windowClose();">';
  	...
  	public function user_list_api(){
  		$userLists = \DB::select(\DB::raw("SELECT email,designation,dept FROM login_plugin_db.login_table WHERE email !='' AND email !='N/A' AND account_status !='blocked'"));
